 ...
 */
public class _055_LychrelNumbers implements Template {
    public static void main(String[] args){
        Util.print(new _055_LychrelNumbers().run());
    }
    public int run() {
        int result = 0;
    ...
    private BigDecimal reverseAndAddBD(BigDecimal num){
        BigDecimal reverseNum = (BigDecimal) Util.reverseNumber(num);
        return num.add(reverseNum);
    }
    private boolean isLychrelNumber(int num){
        BigDecimal numAsBD = new BigDecimal(num);
        for(int iteration = 0; iteration < 50; iteration ++){
            numAsBD = reverseAndAddBD(numAsBD);
            if(Util.isNumberPalindrome(numAsBD)){
                return false;
            }
        }
        return true;
